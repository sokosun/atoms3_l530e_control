...
L530EControl::L530EControl(const IPAddress l530e_ip, const String username, const String password) {
  m_login.ip = l530e_ip;
...
L530EControl::~L530EControl(){}

//-----------------------------------------------------------------------

L530EControl::CipherInfo L530EControl::ParseKey(const String &key) {
...
size_t L530EControl::Cipher(mbedtls_operation_t operation, const uint8_t *input, size_t ilen, uint8_t *output) {
  size_t olen1, olen2;
...
String L530EControl::Encrypt(const String &str) {
  uint8_t buf1[1024];
  uint8_t buf2[1024];
...
String L530EControl::Decrypt(const String &str) {
  uint8_t buf1[1024];
  uint8_t buf2[1024];
...
L530EControl::httpResponse L530EControl::Request(const String &url, const String &payload) {
  int retrycount = 0;
  while(retrycount < 3){
    HTTPClient  http;
    if(!http.begin(url)){
...
bool L530EControl::HasActiveSession() {
...
String L530EControl::GetDeviceInfo() {
  DynamicJsonDocument req(1024);
...
bool L530EControl::SetDeviceInfo(bool on, const L530EControl::HSB & hsb) {
  DynamicJsonDocument req(1024);
  req["method"] = "set_device_info";
  req["params"]["device_on"] = on;
  if(0 <= hsb.hue && hsb.hue < 360){
    req["params"]["hue"] = hsb.hue;
  }
  if(0 <= hsb.saturation && hsb.saturation < 360){
    req["params"]["saturation"] = hsb.saturation;
  }
  if(0 <= hsb.brightness && hsb.brightness <= 100){
    req["params"]["brightness"] = hsb.brightness;
  }
  req["params"]["color_temp"] = 0; // Enable color mode if color temperature is 0
...
bool L530EControl::SetDeviceInfo(bool on, const L530EControl::WhiteLight & wl) {
  DynamicJsonDocument req(1024);
  req["method"] = "set_device_info";
  req["params"]["device_on"] = on;
  if(0 <= wl.brightness && wl.brightness <= 100){
    req["params"]["brightness"] = wl.brightness;
  }
  if(2500 <= wl.temperature && wl.temperature <= 6500){
    req["params"]["color_temp"] = wl.temperature;
  }
